...
export class AzureOpenAIAgent extends BaseAgent {
  ...
  constructor(config: any, toolbox: Toolbox) {
    ...
    this.client = new OpenAI({
      ...
      baseURL: config.baseUrl || `https://${config.resourceName}.openai.azure.com/openai/deployments/${config.deploymentName}`,
  ...
  private async streamProcess(input: AgentInput, observer: Observer<AgentResponse>): Promise<void> {
    try {
      ...
      const stream = await this.client.chat.completions.create({
        model: this.config.model, // This is the deployment name in Azure
