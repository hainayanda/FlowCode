 ...
 * Handles streaming conversations, tool usage, and message formatting specific to Anthropic's API.
 */
export class AnthropicWorker extends BaseWorker {
    private client: Anthropic;

    /**
     * Creates a new AnthropicWorker instance.
    ...
    /**
     * Processes a single iteration using Anthropic's streaming API.
     * Handles real-time text generation and tool usage through Claude's streaming interface.
     *
    ...
    async *singleProcess(
        ...
        return yield* this.finalizeProcess(
            allMessages,
            this.convertToToolCallParameters(toolUses)
        );
    }
    ...
    private convertToToolCallParameters(
        ...
        return toolCalls
            .map((antrhopicToolCall) => {
                if (!antrhopicToolCall.name) {
                    return null;
                }
                if (!antrhopicToolCall.input) {
                    return null;
                }
